...
function ClientProvider({ children }) {
  ...
  async function handleClient(user) {
    ...
    try {
      const machine = await axios.get(
        `http://localhost:3000/clients/${clientId}/devices`
      )
  ...
  async function getClientDashboard(deviceId) {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/clients/${clientId}/devices/${deviceId}`
      )
  ...
  async function machineData(machineId) {
    try {
      const response = await axios.get(
        `http://localhost:3000/devices/${machineId}/history`
      )

      const history = await response.data
      setDeviceData(history)
    } catch (error) {
      if (error.response) {
        // console.log(data)
        // console.log(device)
